...
class TEST_GenerateReviewPlanView(TestCase):
    ...
    def setUpClass(cls):
        ...
        Record.objects.create(user_id=cls.user, word_id=cls.word, last_planed=(
    ...
    def test_0004(self):
        ...
        self.assertEqual(Record.objects.filter(
            user_id=user, last_planed=date.today()).count(), 0)
        response = client.post(self.API_PATH)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Record.objects.filter(
            user_id=user, last_planed=date.today()).count(), 1)
    ...
    def test_0005(self):
        ...
        for i in range(200):
            ...
            Record.objects.create(user_id=user, word_id=word, last_planed=(
                date.today() - timedelta(days=1)))
        client = APIClient()
        client.force_authenticate(user=user)
        # check number of planed reviews is none before making requests
        ...
        response = client.post(self.API_PATH)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Record.objects.filter(user_id=user, last_planed=date.today(
        )).count(), global_param.NUM_REVIEW_RECORDS_PER_DAY)
...
class TEST_NewRecordView(TestCase):
    ...
    def setUp(self):
        ...
        Record.objects.create(user_id=self.user, word_id=word, last_planed=(
            date.today() - timedelta(days=1)))
    ...
    def test_0001(self):
        ...
        requestNumPendingReviews = self.factory.get(
            self.API_PATH_GET_NUM_PENDING_REVIEWS)
        ...
        requestNewRecord = self.factory.post(
            self.API_PATH_NEW_RECORD, data, content_type='application/json')
    ...
    def test_0002(self):
        ...
        # generate plan
        requestGenPlan = self.apiFactory.post(
            self.API_PATH_GENERATE_REVIEW_PLAN)
    ...
    def test_0003(self):
        ...
        self.assertEqual(TagAssignment.objects.filter(
            tag_id__value=data['tag'], user_id=self.user).count(), 0)
        ...
        tagAssignmentSet = TagAssignment.objects.filter(
            tag_id__value=data['tag'], user_id=self.user)
    ...
    def test_0006(self):
        ...
        self.assertEqual(Record.objects.filter(
            word_id__value=data1['word']).count(), 0)
        self.assertEqual(Tag.objects.filter(value=data1['tag']).count(), 0)
        self.assertEqual(TagAssignment.objects.filter(
            tag_id__value=data1['tag'], user_id=self.user).count(), 0)
        self.assertEqual(TagAssignment.objects.filter(
            user_id=self.user).count(), 0)

        requestNewRecord = self.factory.post(
            self.API_PATH_NEW_RECORD, data1, content_type='application/json')
        requestNewRecord.user = self.user
        response = views.NewRecord(requestNewRecord)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        tagAssignmentSet = TagAssignment.objects.filter(
            tag_id__value=data1['tag'], user_id=self.user)
        self.assertEqual(TagAssignment.objects.filter(
            user_id=self.user).count(), 1)
        self.assertEqual(tagAssignmentSet.count(), 1)
        ...
        requestNewRecord = self.factory.post(
            self.API_PATH_NEW_RECORD, data2, content_type='application/json')
        requestNewRecord.user = self.user
        response = views.NewRecord(requestNewRecord)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        tagAssignmentSet = TagAssignment.objects.filter(
            tag_id__value=data2['tag'], user_id=self.user)
        self.assertEqual(TagAssignment.objects.filter(
            user_id=self.user).count(), 2)
        self.assertEqual(tagAssignmentSet.count(), 1)
...
class TEST_SearchRecordsView(TestCase):
    ...
    def setUp(self):
        ...
        requestNewRecord = self.factory.post(
            self.API_PATH_NEW_RECORD, data3, content_type='application/json')
        requestNewRecord.user = self.user
        response = views.NewRecord(requestNewRecord)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_0001(self):
        ...
        requestSearchRecords = self.apiFactory.post(
            self.API_PATH_SEARCH_RECORDS, data)
